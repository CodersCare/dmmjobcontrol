...
class tx_dmmjobcontrol_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
            ...
            if (isset($session['search']) && $search = $session['search']) {
                ...
                foreach ($search AS $field => $value) {
                    ...
                    if (isset($TCA['tx_dmmjobcontrol_job']['columns'][$field]['config']['MM'])) {
                        ...
                        $tableAdd[] = $table;
                        $whereAdd[] = $table.'.uid_local=tx_dmmjobcontrol_job.uid AND ('.$table.'.uid_foreign='.implode(' OR '.$table.'.uid_foreign=', \TYPO3\CMS\Core\Utility\GeneralUtility::removeXSS($value)).')';
        ...
        if (isset($this->piVars['job_uid']) && $this->piVars['job_uid']) {
            ...
            if ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
                ...
                if (isset($this->conf['apply.']['required']) && $this->conf['apply.']['required']) {
                    $this->requiredFields = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['apply.']['required']);
                }
                    ...
                    if (isset($_FILES['tx_dmmjobcontrol_pi1']['name']['apply']['file']) && $_FILES['tx_dmmjobcontrol_pi1']['name']['apply']['file']) {
                        ...
                        foreach ($_FILES['tx_dmmjobcontrol_pi1']['name']['apply']['file'] AS $index => $name) {
                            if ($name) {
                                ...
                                if (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($allowed_file_extensions, strtolower($fileInfo['extension'])) && \TYPO3\CMS\Core\Utility\GeneralUtility::verifyFilenameAgainstDenyPattern($name)) {
